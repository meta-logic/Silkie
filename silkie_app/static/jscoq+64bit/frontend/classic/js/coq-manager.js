...
export class CoqManager {
    ...
    _setupProvider(elems) {
        ...
        console.log("checking elems \n" + elems);
        console.log(this.options.editor);
...
/*             var cm = document.getElementsByClassName("CodeMirror")[4].CodeMirror;
            ...
            // Potential bug when changing lines 
            if (num_goals > 1 && !this.added_subgoals.some(({line : l, text : t}) => l == cursor.line && t == this.doc.stm_id[sid].text && !cm.isReadOnly()))
            {
                let newlines = ""
                let spliced = subgoals.splice(1);

                for (let i = 0; i < spliced.length; i++)
                    {
                        let s = spliced[i].children.length;
                        newlines += " ".repeat(this.bullets.get(sid).length)  + "(* " + spliced[i].children[s-1].textContent +" *)\n"+ " ".repeat(this.bullets.get(sid).length) + this.bullets.get(sid) + "-"  +"\n\n"
                    }
                console.log(newlines);
                
                this.added_subgoals = this.added_subgoals.filter(({line : l, text : t}) => l != cursor.line);
                this.added_subgoals.push({line : cursor.line, text : this.doc.stm_id[sid].text})
                console.log(doc);
                var pos = { 
                        line: cursor.line + 1,
                        ch: 0 
    ...
    async add(stm, tip) {
        ...
        stm.text = stm.text.replace('contradict','discriminate');

        stm.text = stm.text.replace('cases','destruct');
        ...
        if (arr)
        {
            arr = arr.map((x) => x.trim().replace(';',''));
        }

        if (arr && arr.includes('induction'))
        {
            console.log(stm.text);
            
            // Have to loop in case of multiple induction invocations.
            // Might have issue with ";induction"
            let indices = this.getAllIndexes(arr, 'induction');
            
            for (const index of indices)
            {
            let type = null;
            console.log('induction detected at idx ' + index);
            let indvar = arr[index + 1].replace('.','').replace(';','');
            console.log('ind var is ' + indvar);
            let docvars = document.getElementsByClassName("coq-hypothesis");

            for (let i = 0; i < docvars.length && !type; i++)
            {
                let children = docvars[i].childNodes;

                for (let k = 0; k < children.length; k++)
                {
                    if (children[k].textContent.trim() == indvar)
                    {
                        type = children[children.length - 1].textContent;
                        console.log("found var of type " + type);
                        break;
                    }
                }
            }
            ...
            if (!type)
            {
                console.log("looking through context");
                let envir = document.getElementsByClassName("coq-env")[0];
                if (typeof envir !== 'undefined')
                {
                    let etext = envir.textContent;
                    let quants = etext.slice(0, etext.indexOf(','));
                    console.log(quants);

                    let typedefs = quants.match(/\(([^()]*)\)/g);
                    if (typedefs)
                    {
                        typedefs = typedefs.map(function($0) { return $0.substring(1,$0.length-1) });
                    }

                    else 
                    {
                        typedefs = [quants.slice(1, quants.length)];
                    }
                
                    for (let i = 0; typedefs && i < typedefs.length && !type; i++)
                    {
                        let spl = typedefs[i].split(':');
                        let lvars = spl[0].split(' ').map((x) => x.trim());
                        let tp = spl[1].trim();
    
                        console.log(lvars);
                        console.log("their type is " + tp);
    
                        for (const vv of lvars)
                            {
                                if (vv == indvar)
                                {
                                    console.log("found indvar with type " + tp);
                                    type = tp;
                                    break;
                                }
                            }
            ...
            // Check for the other pre-defined types like R and Z
            if (type && type != 'ℕ' && type != 'Z' && !type.includes("list"))
            {
                stm.text = stm.text.replace('induction ' + indvar, 'induction ' + indvar + ' using ' + type + '_ind_princip_proof');
            }   
        }
        ...
        let trimm = stm.text.trim();
        let firstL = trimm.charAt(0);
        let lastL = trimm.charAt(trimm.length - 1);

        if (firstL == lastL && firstL == "-")
        {
            console.log("found a bullet");
            this.bullets.set(stm.coq_sid, trimm);
        }

        else {
            if (!this.bullets.get(tip))
            {
                this.bullets.set(stm.coq_sid, "");
            }

            else {
                this.bullets.set(stm.coq_sid, this.bullets.get(tip));
            }
        }
